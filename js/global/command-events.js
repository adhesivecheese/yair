...
(function () {
	...
	eventCmd.send = function (target, path, params, success, error) {
		...
		var event = new CustomEvent('EventCmd', {
			detail: eventData
		});
	...
	var response = {
		...
		, send: function (cmdKey, success, args) {
			...
			var responseEvent = new CustomEvent('EventResponse', {
				detail: returnData
			});
	...
	window.addEventListener('EventResponse', function (e) {
		...
		if (e.detail.success) {
			if (typeof responseCallback.success === "function") responseCallback.success.apply(null, e.detail.args);
		}
		else {
			if (typeof responseCallback.error === "function") responseCallback.error.apply(null, e.detail.args);
		}
	...
	eventCmd.ping = function (sender) {
		...
		var responseEvent = new CustomEvent('EventCmdPong', {
			detail: eventCmd.extensionNamespace
		});
		window.dispatchEvent(responseEvent);
	};
